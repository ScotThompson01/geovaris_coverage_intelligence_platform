  ...
  try {
    ...
    const rows = await sql`
      SELECT
        ...
        ST_AsGeoJSON(
          ST_ForcePolygonCCW(cr.coverage_geometry)
        )::json AS coverage_geometry,
        s.name AS site_name
      FROM coverage_runs cr
      JOIN scenarios sc
        ON sc.id = cr.scenario_id
        AND sc.customer_id = cr.customer_id
      JOIN sites s
        ON s.id = sc.site_id
        AND s.customer_id = cr.customer_id
      WHERE cr.status = 'completed'
        AND cr.coverage_geometry IS NOT NULL
        AND cr.scenario_id = ${scenarioId}
    ...
    return NextResponse.json(
      {
        status: "error",
        error: "Unable to load coverage result.",
      },
      { status: 500 },
    );
  }
