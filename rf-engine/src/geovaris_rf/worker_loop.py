...
def process_one_available_run() -> bool:
    ...
    Rapid Coverage is checked first because it is the current
    GeoVaris MVP coverage workflow.

    NTIA ITM and free-space testing remain available as separate
    development processing paths.

    Returns True if any worker processed a run.
    Returns False if no supported pending run was available.
    """

    if process_one_rapid_run():
        return True
...
def run_worker_loop() -> None:
    ...
    print(
        "GeoVaris RF development worker started."
    )

    print(
        "Supported models: "
        "rapid_coverage, ntia_itm, free_space_test"
    )

    print(
        f"Polling every "
        f"{POLL_INTERVAL_SECONDS:.1f} seconds."
    )
    ...
    try:
        ...
        print()
        print(
            "GeoVaris RF development worker stopped."
        )
